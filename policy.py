...
class QLearningPolicy(Policy):
    ...
    * :attr:`...` You can add additional attributes for this class.
    ...
    def __init__(self, marker, board_size=(3, 3)):
        ...
        ########################################## YOUR CODE HERE ##########################################
        # TODO
        # You can add additional attribute here
        # You can also adjust the initial_val here
    ...
    def decide(self, state):
        ...
        # Note: You can directly loop up the value of Q(state, action) with self.q_table[state, action].



        for action in available_actions:
            if self.q_table[state, action] > self.q_table[state, move]:
                move = action
        
        # epsilon-greedy strategy
        if self.mode == "train" and np.random.rand() < self.epsilon:
            move = available_actions[np.random.randint(len(available_actions))]
    ...
    def update_q_table(self, state, action, reward, next_state):
        ...
        # Note: You can directly set the value Q(state, action) to val with self.q_table[state, action] = val.
        

        if next_state is None:
            future_max_q = 0
        else:
            future_max_q =  self.q_table[next_state, self.decide(next_state)]
        
        newQ = self.q_table[state, action] + self.alpha * (reward + self.gamma * future_max_q - self.q_table[state, action])
